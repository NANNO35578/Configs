...
// Windows Terminal
item(where=package.exists("WindowsTerminal") title="Open Terminal Here" image='@package.path("WindowsTerminal")\WindowsTerminal.exe' cmd='wt.exe' arg='-d "@sel.path\."')

modify(find='Group by' parent=title.more_options)
modify(find='View' parent=title.more_options)
modify(find='Sort by' parent=title.more_options)

item(title="Open in Everything" image="E:\\Everything\\Everything.exe" cmd="E:\\Everything\\Everything.exe" arg='-path "@sel.path"')
